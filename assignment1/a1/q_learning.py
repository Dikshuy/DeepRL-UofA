...
class QLearning:
    ...
    def update_q(self, obs, action, reward, next_obs, terminated):
        # start your code
        self.q[obs, action] += self.step_size * (reward + self.discount * (1 - terminated) * np.max(self.q[next_obs]) - self.q[obs, action])
        # end your code 
    ...
    def process_transition(self, obs: int, reward: float, terminated: bool, truncated: bool) -> None:
        ...
        next_state = obs
        self.update_q(state, action, reward, next_state, terminated) # keep this line
        # End your code here
